...
?>
<div class="bx_filter col-3 filters">
	<div class="bx_filter_section filters__inner">
		<h2>Фильтры</h2>
				...
				if(isset($arItem["PRICE"])):
                    ...
                    <div class="filter-price">
                        <div class="filter-price__inputs">
                            <input type="text" class="filter-price__min" name="<?echo $arItem["VALUES"]["MIN"]["CONTROL_NAME"]?>"  id="<?echo $arItem["VALUES"]["MIN"]["CONTROL_ID"]?>" value="<?echo (int)$arItem["VALUES"]["MIN"]["VALUE"]?>" onkeyup="smartFilter.keyup(this)">
                            <p>&mdash;</p>
                            <input type="text" class="filter-price__max" name="<?echo $arItem["VALUES"]["MAX"]["CONTROL_NAME"]?>"  id="<?echo $arItem["VALUES"]["MAX"]["CONTROL_ID"]?>" value="<?echo (int)$arItem["VALUES"]["MAX"]["VALUE"]?>" onkeyup="smartFilter.keyup(this)">
                        </div>
                        <div class="filter-price__range">
                            <input type="text" class="js-range-slider" name="my_range" value="1" />
                        </div>
                    </div>

<?//var_dump($arItem["VALUES"]["MIN"]["VALUE"])?><br> 
<?//print_r($arItem["VALUES"]["MAX"]["VALUE"])?>               
<script type="text/javascript">
<?	if(isset($arItem["VALUES"]["MIN"]["VALUE"])):?>
  var min_price = <?=json_encode((int)$arItem["VALUES"]["MIN"]["VALUE"]); ?>; 
  var max_price = <?=json_encode((int)$arItem["VALUES"]["MAX"]["VALUE"]); ?>; 
<?else:?>
  var min_price = 0;
  var max_price = 0;
<?endif;?>
  var price_from = min_price;
  var price_to = max_price;
  var catalog = true;
...
<?if (isset($_GET['arrFilter_P1_MIN'])):?>
	<script type="text/javascript">
	  var price_from = <?=json_encode((int)$_GET['arrFilter_P1_MIN']); ?>; 
	  var price_to =   <?=json_encode((int)$_GET['arrFilter_P1_MAX']); ?>; 
	</script>
<?endif?>
		...
		<script type="text/javascript">
		  ...
		  var catalog = true;
				...
				<div class="bx_filter_parameters_box <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>active<?endif?>">
					...
					<!-- <div class="bx_filter_block"> -->
						...
						switch ($arItem["DISPLAY_TYPE"])
						{
								...
								<div class="bx_filter_select_container">
									<div class="bx_filter_select_block" onclick="smartFilter.showDropDownPopup(this, '<?=CUtil::JSEscape($key)?>')">
										...
										<?foreach ($arItem["VALUES"] as $val => $ar):?>
											...
											/>
										<?endforeach?>
										<div class="bx_filter_select_popup" data-role="dropdownContent" style="display: block;">
											<ul>
												<li>
							...
							default://CHECKBOXES
								?>
								<div class="filter-manuf-series">
								<?foreach($arItem["VALUES"] as $val => $ar):?>
									<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="fancy-checkbox <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
											...
											<span class=" fancy-checkbox__checkmark" title="<?=$ar["VALUE"];?>"></span><?=$ar["VALUE"];?>
			...
			<!-- <div class="bx_filter_button_box active"> -->
				<!-- <div class="bx_filter_block"> -->
					<!-- <div class="bx_filter_parameters_box_container"> -->
<!-- <a href="#" class="btn btn_main btn_size-l full-width text-center">Применить</a>
<a href="#" class="btn btn_secondary btn_size-l full-width text-center">Очистить фильтр</a>	 -->	
		<input class="btn_size-l full-width btn_main" type="submit" id="set_filter" name="set_filter" value="<?=GetMessage("CT_BCSF_SET_FILTER")?>" 
		style="margin-bottom: 10px; display: block;"/><br>
		<input class="btn_size-l full-width btn_secondary" type="submit" id="del_filter" name="del_filter" value="Очистить фильтр" />
		...
		<div style="clear: both;"></div>
		<a href="#" class="ad">
	        <div class="ad__inner" style="background-image: url(/img/bg/bg-banner.png);"></div>
	    </a>		
	</div>
